...
int parseQueryString()
{
    ...
    if (query_string != NULL)
    {
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            /* +4 to remove sh1= sp1= ... */
            strcpy(clients[i].sock_ip, strtok(NULL, "&") + 4);
            strcpy(clients[i].sock_port, strtok(NULL, "&") + 4);

            /* active only if host, port, file all complete */
            clients[i].is_active = (strlen(clients[i].host_ip) && strlen(clients[i].host_port) && strlen(clients[i].batch_file));
            /* is_socks only if socks host, socks port all complete */
            clients[i].is_socks = (strlen(clients[i].sock_ip) && strlen(clients[i].sock_port));
        }
    }
    return 0;
}
...
int connectServer()
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        if (clients[i].is_active)
        {
            ...
            hostname = gethostbyname(clients[i].is_socks ? clients[i].sock_ip : clients[i].host_ip);
            if (hostname == NULL)
            {
            ...
            if (clients[i].is_socks)
            {
                unsigned char socks4_request[8], socks4_reply[8];
                short dstport = atoi(clients[i].host_port);
                char dstip[16];

                socks4_request[0] = 4;
                socks4_request[1] = 1;
                socks4_request[2] = (dstport >> 8) & 0xFF;
                socks4_request[3] = dstport & 0xFF;

                struct hostent *he = NULL;
                if ((he = gethostbyname(clients[i].host_ip)) == NULL)
                {
                    fprintf(stderr, "gethostbyname error:");
                    perror(strerror(errno));
                }
